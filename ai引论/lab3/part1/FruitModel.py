    ...
    def count(self):
        ...
        for text,label in self.dataset:
            if label==1:
                ...
                for tokens in text:
                    if tokens not in self.token_num[0] and tokens not in self.token_num[1]:
                        self.V+=1
                    if tokens in self.token_num[0]:
                        self.token_num[0][tokens]+=1
                    else:
                        self.token_num[0][tokens]=1
            else:
                ...
                for tokens in text:
                    if tokens not in self.token_num[0] and tokens not in self.token_num[1]:
                        self.V+=1
                    if tokens in self.token_num[1]:
                        self.token_num[1][tokens]+=1
                    else:
                        self.token_num[1][tokens]=1
        self.total_words = [sum(self.token_num[0].values()), sum(self.token_num[1].values())]

    def __call__(self, text):
        # TODO: YOUR CODE HERE
        # 返回1或0代表当前句子分类为正/负样本
        total_file=self.pos_neg_num[0]+self.pos_neg_num[1]
        p_pos=np.log(self.pos_neg_num[0]/total_file)
        p_neg=np.log(self.pos_neg_num[1]/total_file)
        for tokens in text:
            count_pos=self.token_num[0].get(tokens,0)
            p_pos+=np.log((count_pos+1)/(self.total_words[0]+self.V))
            count_neg=self.token_num[1].get(tokens,0)
            p_neg+=np.log((count_neg+1)/(self.total_words[1]+self.V))
        if p_pos>p_neg:
            return 1
        else:
            return 0
...
class QAModel():
    ...
    def idf(self, word):
        ...
        # 注意python整除与整数除法的区别
        total=len(self.document_list)
        cnt=0
        for documents in self.document_list:
    ...
    def __call__(self, query):
        ...
        for documents in self.document_list:
            doc_score=0.0
            for words in query:
                doc_score+=self.tfidf(words, documents)
            if doc_score > max_doc_score:
                max_doc_score=doc_score
                best_doc=documents
        best_sentence=""
        max_sent_score=-1.0
        for sent_data in best_doc['sentences']:
            sent_tokens=sent_data[0]   
            sent_original_text=sent_data[1] 
            sent_score=0.0
            for words in query:
                if words in sent_tokens:
                    sent_score+=self.idf(words)
            if sent_score > max_sent_score:
                max_sent_score=sent_score
                best_sentence=sent_original_text
